...
impl NodeMethods for NodeTypes {
                    ...
                    ctx.map(|c| {
                        if c == "DEBUG" {
                            format!(
                                ...
                                &v.max,
                                v._val
                                    .map_or_else(|| "".to_string(), |val| format!("={}", val))
                            )
                        } else if c == "REPR" {
                            format!(
                                "Variable('{}', {:?}, {:?}){}",
    ...
    fn vars(&self) -> Vec<&NodeTypes> {
        match self {
            ...
            NodeTypes::SumNode(n) | NodeTypes::AndNode(n) => {
                n.nodes.iter().flat_map(NodeTypes::vars).collect()
            }
            _ => vec![],
        }
    }
    fn substitute(&self, var_vals: &HashMap<NodeTypes, NodeTypes>) -> NodeTypes {
        match self {
            ...
            NodeTypes::LtNode(l) => match l.b.as_ref() {
                BTypes::Int(i) => l.a.substitute(var_vals).n2i_lt(i),
                BTypes::Node(n) => l.a.substitute(var_vals).n2n_lt(&n.substitute(var_vals)),
            },
            NodeTypes::MulNode(m) => match m.b.as_ref() {
                BTypes::Int(i) => m.a.substitute(var_vals) * *i,
                BTypes::Node(n) => m.a.substitute(var_vals) * n.substitute(var_vals).clone(),
            },
            NodeTypes::DivNode(d) => {
                d.a.substitute(var_vals)
                    .floordiv(d.b.as_ref().clone(), true)
            }
            NodeTypes::ModNode(m) => match m.b.as_ref() {
                BTypes::Int(i) => m.a.substitute(var_vals) % *i,
                BTypes::Node(n) => m.a.substitute(var_vals) % n.clone(),
            },
            NodeTypes::SumNode(s) => {
                Self::sum(s.nodes.iter().map(|n| n.substitute(var_vals)).collect())
            }
            NodeTypes::AndNode(a) => {
                Self::ands(&a.nodes.iter().map(|n| n.substitute(var_vals)).collect())
            }
        }
    }
    fn unbind(&self) -> (NodeTypes, Option<f64>) {
        match self {
            ...
            _ => {
                ...
                self.vars().into_iter().for_each(|v| match v {
    ...
    fn sum(nodes: Vec<NodeTypes>) -> NodeTypes {
        ...
        for nodes in &NodeTypes::new_sum(nd).flat_components() {
            match nodes {
                ...
                NodeTypes::MulNode(n) => match n.b.as_ref() {
                    BTypes::Node(b_n) => {
                        let getter = mul_groups
                            .get(&n.a.clone())
                            .unwrap_or_else(|| &BTypes::Int(0.0));
                        match getter {
                            BTypes::Int(i) => {
                                mul_groups.insert(
                                    n.a.as_ref().clone(),
                                    BTypes::Node(i.clone() + b_n.clone()),
                                );
                            }
                            BTypes::Node(bb_n) => {
                                mul_groups.insert(
                                    n.a.as_ref().clone(),
                                    BTypes::Node(bb_n.clone() + b_n.clone()),
                                );
                            }
                        }
                    }
                    BTypes::Int(i) => {
                        ...
                        match getter {
                            BTypes::Int(ii) => {
                                mul_groups
                                    .insert(n.a.as_ref().clone(), BTypes::Int(ii + i.clone()));
                            }
                            BTypes::Node(bb_n) => {
                                mul_groups.insert(
                                    n.a.as_ref().clone(),
                                    BTypes::Node(bb_n.clone() + i.clone()),
                                );
                            }
                        }
                    }
...
impl Into<bool> for NodeTypes {
    fn into(self) -> bool {
        match self {
            ...
            | NodeTypes::LtNode(a)
            | NodeTypes::ModNode(a)
            | NodeTypes::MulNode(a) => !(a.min == a.max && a.min == 0.0),
            other @ _ => !(other.min() == other.max() && other.min() == 0.0),
        }
    }
}
...
impl Mul<NodeTypes> for NodeTypes {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        match &self {
            ...
            NodeTypes::MulNode(n) => match n.b.as_ref() {
                BTypes::Int(i) => n.a.as_ref().to_owned() * (i.to_owned() * rhs),
                BTypes::Node(nn) => n.a.as_ref().to_owned() * (nn.to_owned() * rhs),
            },
            NodeTypes::SumNode(s) => NodeTypes::sum(
                s.nodes
                    .iter()
                    .map(|n| n.clone() * rhs.borrow().to_owned())
                    .collect::<Vec<NodeTypes>>(),
            ),
            _ => match rhs {
                NodeTypes::NumNode(n) => create_node(NodeTypes::new_mul(self, BTypes::Int(n.b))),
                _ => create_node(NodeTypes::new_mul(self, BTypes::Node(rhs))),
            },
        }
    }
}
...
impl Mul<f64> for NodeTypes {
    ...
    fn mul(self, rhs: f64) -> Self::Output {
        match &self {
            ...
            NodeTypes::MulNode(n) => match n.b.as_ref() {
                BTypes::Int(i) => n.a.as_ref().to_owned() * (i * rhs),
                BTypes::Node(nn) => n.a.as_ref().to_owned() * (nn.to_owned() * rhs),
            },
            NodeTypes::SumNode(s) => NodeTypes::sum(
                s.nodes
...
impl std::ops::Rem<NodeTypes> for NodeTypes {
    ...
    fn rem(self, rhs: Self) -> Self::Output {
        match &rhs {
            ...
            NodeTypes::MulNode(n) => match n.b.as_ref() {
                BTypes::Int(i) => n.a.as_ref().to_owned() * (i.to_owned() % rhs.clone()) % rhs,
                BTypes::Node(i) => n.a.as_ref().to_owned() * (i.to_owned() % rhs.clone()) % rhs,
            },
            ...
            NodeTypes::SumNode(s) => {
                ...
                if (rhs.clone() - self.clone()).min() > 0.0 {
                    return self;
                }
                let mut result = vec![];
                s.nodes.iter().for_each(|n| match n {
                    NodeTypes::NumNode(_) => {
            ...
            _ => {
                ...
                if (rhs.clone() - self.clone()).min() > 0.0 && self.min() >= 0.0 {
                    self
                } else {
...
impl std::ops::Rem<f64> for NodeTypes {
    ...
    fn rem(self, rhs: f64) -> Self::Output {
        match &self {
            NodeTypes::MulNode(n) => match n.b.as_ref() {
                BTypes::Int(i) => n.a.as_ref().to_owned() * (i % rhs) % rhs,
                BTypes::Node(i) => n.a.as_ref().to_owned() * (i.to_owned() % rhs.clone()) % rhs,
            },
            NodeTypes::SumNode(s) => {
                if self == rhs {
            ...
            NodeTypes::ModNode(n) => match n.b.as_ref() {
                BTypes::Int(i) => {
                    if i % rhs == 0.0 {
            ...
            _ => {
                ...
                if self.min() >= 0.0 && self.max() < rhs.clone() {
                    return self.clone() - (rhs.clone() * ((self.min() / rhs.to_owned()).floor()));
                }
                if self.min() < 0.0 {
                    return (self.clone() - (((self.min() / rhs.clone()).floor()) * rhs.clone()))
                        % rhs.to_owned();
                }
...
fn sym_render(a: &BTypes, ops: &Option<Rc<dyn Any>>, ctx: Option<&str>) -> String {
    match &a {
        ...
        BTypes::Node(n) => n.render(ops, &ctx),
    }
}

fn render_mulnode(m: &OpNode, ops: &Option<Rc<dyn Any>>, ctx: &Option<&str>) -> String {
    match m.a.as_ref() {
        NodeTypes::Variable(v_a) => match m.b.as_ref() {
            BTypes::Node(n) => match n {
...
impl NodeTypes {
    ...
    fn min(&self) -> f64 {
        match self {
            ...
            | NodeTypes::LtNode(n)
            | NodeTypes::ModNode(n)
            | NodeTypes::MulNode(n) => n.min,
            NodeTypes::AndNode(n) | NodeTypes::SumNode(n) => n.min,
            NodeTypes::NumNode(n) => n.min,
            NodeTypes::Variable(n) => n.min,
        }
    }
    ...
    fn max(&self) -> f64 {
        match self {
            ...
            | NodeTypes::LtNode(n)
            | NodeTypes::ModNode(n)
            | NodeTypes::MulNode(n) => n.max,
            NodeTypes::AndNode(n) | NodeTypes::SumNode(n) => n.max,
            NodeTypes::NumNode(n) => n.max,
            NodeTypes::Variable(n) => n.max,
        }
    }
    ...
    fn floordiv(&self, b: BTypes, factoring_allowed: bool) -> Self {
        match self {
                ...
                BTypes::Int(i) => match &b {
                    ...
                    BTypes::Node(nn) => {
                        if i.clone() % nn.clone() == 0.0 {
                            ...
                            n.a.floordiv(
                                BTypes::Node(
                                    nn.floordiv(n.b.as_ref().to_owned(), factoring_allowed),
                                ),
                                true,
                            )
                        } else {
                ...
                BTypes::Node(i) => match &b {
                    BTypes::Int(ii) => {
                        if i.clone() % ii.to_owned() == 0.0 {
                            ...
                            n.a.floordiv(
                                BTypes::Node(NodeTypes::rfloordiv(*ii, n.b.as_ref().to_owned())),
                                true,
                            )
                        } else {
                            self.floordiv(b, factoring_allowed)
            ...
            NodeTypes::DivNode(n) => match n.b.as_ref() {
                BTypes::Int(i) => match &b {
                    BTypes::Int(ii) => n.a.floordiv(BTypes::Int(ii * i), factoring_allowed),
                    BTypes::Node(ii) => n.a.floordiv(
                        BTypes::Node(ii.to_owned() * i.to_owned()),
                        factoring_allowed,
                    ),
                },
                BTypes::Node(i) => match &b {
            ...
            NodeTypes::ModNode(n) => match &b {
                ...
                BTypes::Node(i) => match n.b.as_ref() {
                    BTypes::Int(ii) => {
                        if ii.clone() % i.to_owned() == 0.0 {
                            n.a.floordiv(b.to_owned(), factoring_allowed)
                                % NodeTypes::rfloordiv(ii.to_owned(), b)
                        } else {
                            self.floordiv(b, factoring_allowed)
                        }
                    }
                    BTypes::Node(nn) => {
                        if nn.to_owned() % i.to_owned() == 0.0 {
                            n.a.floordiv(b.clone(), factoring_allowed)
                                % nn.floordiv(b, factoring_allowed)
                        } else {
                            self.floordiv(b, factoring_allowed)
                        }
                    }
            ...
            NodeTypes::SumNode(_) => match &b {
                BTypes::Int(i) => {
                    ...
                    let mut divisor = 1.0;
                    self.flat_components().iter().for_each(|n| match n {
                        NodeTypes::NumNode(x) => {
                            if x.b % i.clone() == 0.0 {
                        ...
                        NodeTypes::MulNode(x) => match x.b.as_ref() {
                            BTypes::Int(ii) => {
                                if ii % i == 0.0 {
                                    fully_divided.push(n.floordiv(b.to_owned(), factoring_allowed));
                                } else {
                    ...
                    if _gcd > 1.0 {
                        return (NodeTypes::sum(fully_divided)
                            + NodeTypes::sum(rest).floordiv(BTypes::Int(_gcd), factoring_allowed))
                        .floordiv(BTypes::Int((i / _gcd).floor()), factoring_allowed);
                    }
                    if divisor > 1.0 {
                        return (NodeTypes::sum(fully_divided)
                            + NodeTypes::sum(rest)
                                .floordiv(BTypes::Int(divisor), factoring_allowed))
                        .floordiv(BTypes::Int((i / divisor).floor()), factoring_allowed);
                    } else {
                        return NodeTypes::sum(fully_divided)
                            + NodeTypes::sum(rest).floordiv(b, factoring_allowed);
                    }
                }
                BTypes::Node(n_b1) => {
                    if self.clone() == n_b1.clone() {
                        return NumNode::init(1.0);
                    }
                    let mut fully_divided: Vec<NodeTypes> = vec![];
                    ...
                    if sum_fully_divided != 0.0 {
                        return (sum_fully_divided + create_node(NodeTypes::new_sum(rest)))
            ...
            _ => match &b {
                BTypes::Node(n) => match n {
                    NodeTypes::NumNode(num) => self.floordiv(BTypes::Int(num.b), factoring_allowed),
                    _ => {
                        if self == n {
                            NumNode::init(1.0)
                ...
                BTypes::Int(i) => {
                    ...
                    if self.min() < 0.0 {
                        ...
                        return (self.clone() + -offset.borrow() * i.to_owned()).floordiv(b, false)
                            + offset;
                    }
    ...
    fn n2n_lt(&self, other: &NodeTypes) -> NodeTypes {
        match self {
            NodeTypes::MulNode(n) => self.n2n_le(&(other.clone())),
            NodeTypes::SumNode(s) => self.n2n_lt(other),
            _ => create_node(Self::new_lt(self.clone(), BTypes::Node(other.clone()))),
        }
    }
    fn n2i_lt(&self, other: &f64) -> NodeTypes {
        match self {
            NodeTypes::MulNode(n) => match n.b.as_ref() {
                BTypes::Node(_) => self.n2i_lt(&other.clone()),
                BTypes::Int(i) => {
                    if *i == -1.0 {
                    ...
                    (n.a.as_ref().to_owned() * sgn)
                        .n2i_lt(&((other + i.abs() - 1.0) / i.abs()).floor())
                }
            },
            NodeTypes::SumNode(s) => {
                ...
                let (muls, others) = partition(nodes, |x| {
                    if let NodeTypes::MulNode(m) = x {
                        match m.b.as_ref() {
                            BTypes::Int(i) => {
                                if i.to_owned() > 0.0 && x.max() >= other.to_owned() {
                                    return true;
                                } else {
                                    return false;
                                }
                            ...
                            BTypes::Node(n) => {
                                if n.n2i_gt(0.0.borrow()).into() && x.max() >= other.to_owned() {
                                    return true;
                                } else {
                                    return false;
                                }
                ...
                muls.iter().for_each(|x| match x {
                    ...
                    NodeTypes::NumNode(n) => {
                        mul_gcd = gcd(
                            mul_gcd.floor().to_isize().unwrap(),
                            n.b.floor().floor().to_isize().unwrap(),
                        )
                        .to_f64()
                        .unwrap();
                    }
    ...
    fn flat_components(&self) -> Vec<NodeTypes> {
        match self {
            NodeTypes::SumNode(s) => {
                let mut result = vec![];
                s.nodes.iter().for_each(|x| match x {
                    NodeTypes::SumNode(_) => {
